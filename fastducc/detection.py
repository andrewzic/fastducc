        ...
        if std_mode == "spatial_per_window":
            ...
            if use_local_threshold:
                for t0 in range(T_eff):
                    mu_loc, sigma_loc = kernels._local_mean_std_2d(
                        S[t0], valid_mask=valid_mask, window_size=local_window_size
                    )
                    snr[t0, :, :] = (S[t0, :, :] - mu_loc) / sigma_loc
...
def variance_search(
    ...
    *,
    ...
    # 2) spatial sigma / SNR map (2-D)
    if use_local_threshold:
        mu_local, sigma_local = kernels._local_mean_std_2d(
            std_map, valid_mask=valid_mask, window_size=local_window_size
        )
        snr = (std_map - mu_local) / sigma_local
    else:
        if spatial_estimator == "mad":
...
def variance_search_welford(
    ...
    *,
    ...
    if use_local_threshold:
        mu_local, sigma_local = kernels._local_mean_std_2d(
            std_map64, valid_mask=valid_mask, window_size=local_window_size
        )
        if subtract_mean_of_std_map:
            snr = (std_map64 - mu_local) / sigma_local
        else:
            snr = std_map64 / sigma_local
    else:
        if spatial_estimator == "mad":
    ...
    *,
    ...
    for h, H in enumerate(Hcubes, start=1):
        ...
        for k in range(Nk):
            ...
            if use_local_threshold:
                mu_loc, sigma_loc = kernels._local_mean_std_2d(
                    img, valid_mask=mask, window_size=local_window_size
                )
                snr_map = (img - mu_loc) / sigma_loc
            else:
                if spatial_estimator == "mad":
